---
title: "ACC23 Visium HD - 008um Bins"
...
artifacts <- read.csv(paste0(result_dir, "ACC23_Artifacts.csv"), header=TRUE)

object[["artifact"]] <- artifacts$Annotation[match(rownames(object@meta.data), artifacts$Barcode)]
object[["non_artifact"]] <- is.na(object[["artifact"]])  # artifacts are marker "fold_x", non-artifacts are NA
```
...
```{r, fig.width=8, fig.height=6}
object <- subset(object, subset=nFeature_Spatial.008um >= 100 & percent.mito < 25 & non_artifact)
...
object <- FindClusters(object, cluster.name="seurat_cluster", resolution=0.6)
...
cluster_pal <- c("#22A889", "#0068A9", "#FCA50B", "#C23637", "#7AD151", "#35B7CE", "#E96302", 
                 "#932667", "#92C4E9", "#CDA0CB", "#F5D77E", "lightcoral", "#BBDF70", 
...
# Remove small outlier cluster (clusters 13 contains only 2 cells)
object <- subset(object, seurat_cluster %in% paste0("C", 0:12))
...
dat_summary <- dat_summary %>%
  mutate(Description=case_when(Cluster == "C0" ~ "Tumor",
                               Cluster == "C1" ~ "Tumor",
                               Cluster == "C2" ~ "Tumor (S Phase)",
                               Cluster == "C3" ~ "Tumor (High B2M)",
                               Cluster == "C4" ~ "Mixed Immune / Fibroblast",
                               Cluster == "C5" ~ "Macrophage",
                               Cluster == "C6" ~ "Tumor (Hypoxic)",
                               Cluster == "C7" ~ "Tumor",
                               Cluster == "C8" ~ "Endothelial",
                               Cluster == "C9" ~ "Tumor",
                               Cluster == "C10" ~ "B Cell",
                               Cluster == "C11" ~ "Skeletal Muscle",
                               Cluster == "C12" ~ "Tumor"))

dat_summary %>% arrange(desc(`Mean B2M Expr`)) %>% knitr::kable()

write.table(dat_summary %>% arrange(Cluster), 
            paste0(result_dir, "ACC23_cluster_stats.tsv"), sep="\t", row.names=FALSE)
```
...
write.table(top_marker_df, paste0(result_dir, "ACC23_top_cluster_marker_genes.tsv"), sep="\t", row.names=FALSE)
```
...
heatmap_genes <- c("ACTC1", "TTN", "DES",
                   "VEGFA", "NDRG1", "SLC2A1",
                   "KRT6B", "MMP7", "GABRP",
                   "ABCA13", "AZGP1", "EHF",
                   "FCGBP", "C1QC", "CD14",
                   # C7 defined by lack of specific marker expression 
                   "TNXB", "LTBP4", "TGFBR2",
                   "LGR6", "PLAT", "APOE",
                   "HIST1H1B", "TOP2A", "HIST1H1D",
                   "TNNI2", "FOS", "CDKN1A",
                   "IGKC", "IGHG1", "IGHA1",
                   "COL6A3", "FN1", "CD74",
                   "PECAM1", "RGS5", "ENG")
...
p <- DoHeatmap(subset(object, downsample=1000), slot="scale.markers", assay="Spatial.008um", 
  ...
  theme(legend.title=element_text(face="bold"), 
        ...
        axis.text.y=element_text(size=12, face="bold"), legend.key.size=unit(0.4, "cm"), legend.position="bottom")
...
p + scale_color_gradientn(colors=c("#DDDDDD", "red")) + labs(x="UMAP 1", y="UMAP 2") + guides(color="none")
```
...
```{r}
object[["group"]] <- ifelse(object$seurat_cluster %in% c("C3"), "HighB2M",
                            ifelse(object$seurat_cluster %in% c("C0", "C1", "C2", "C6", "C7", "C9", "C12"), "LowB2M", "Other"))
...
write.table(sig_res, paste0(result_dir, "ACC23_high_vs_low_B2M_tumor_clusters_DE_sig_res.tsv"), sep="\t", row.names=FALSE)
```

```{r, fig.width=8, fig.height=8}
Idents(object) <- "seurat_cluster"
SpatialDimPlot(subset(object, seurat_cluster %in% c("C3", "C0", "C1", "C2", "C6", "C7", "C9", "C12")), repel=T, 
               label.size=7, image.scale="hires", pt.size.factor=5, shape=22, image.alpha=0.5) +
...
res <- res %>% mutate(Category=ifelse(p_val_adj < 0.05 & avg_log2FC > 0.5, "Upregulated", 
                                      ifelse(p_val_adj < 0.05 & avg_log2FC < -0.5, "Downregulated", 
                                             "Not Significant")),
                      Label=ifelse(SYMBOL %in% c("IRF1", "TAP1", "GBP1"),
                                   SYMBOL, NA),
...
ggplot(res, aes(x=avg_log2FC, y=-log10(p_val_adj))) +
  ...
  labs(x="Log2FC", y="-Log10(Adj p-Val)", size="% Cells Detected (C3)") +
...
for (term in c("response to type II interferon", "adaptive immune response", "innate immune response")) {
  gseaplot2(gseaGO_bp, geneSetID=which(gseaGO_bp$Description == term), color="#74C465", base_size=11.5) %>% print()
}
...
dotplot(gseaGO_bp_dedup, showCategory=20, x="enrichmentScore") + 
